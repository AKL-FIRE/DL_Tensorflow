...
class AdditiveGaussianNoiseAutoencoder(object):
    def __init__(self, n_input, n_hidden, transfer_function=tf.nn.softplus, optimizer=tf.train.AdamOptimizer(), scale=0.1):
        ...
        self.hidden = self.transfer(tf.matmul(self.x + scale * tf.random_normal([n_input]), self.weights['w1']) + self.weights['b1'])
...
def test_autoencoder(index):
    ...
    plt.imshow(reshaped)
    plt.show()
    image_trans = tf.reshape(image, [1, 784])
    image_trans = sess.run(image_trans)
    code = autoencoder.transform(image_trans)
    decode = autoencoder.generate(code)
    after_image = tf.reshape(decode, [28, 28])
    after_image = sess.run(after_image)
    plt.figure('after')
